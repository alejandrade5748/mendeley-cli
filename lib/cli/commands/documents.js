...
export function register(program) {
  ...
  Use \`mendeley catalog search\` to search the global catalog.`,
    ...
    .action(async ([query], flags, out) => {
      ...
      if (flags.all) {
        const items = await collect(search.iter(kwargs));
        out.writeList(items);
      } else {
  ...
  The --data / --file JSON body recognises the same field names the
  Mendeley API uses.  Commonly used keys (#104):

    • title       (string, required)
    • type        (string, e.g. journal, book, generic — required)
    • year        (integer)
    • authors     (array of {first_name, last_name} objects; or
                   strings — 'First Last', 'Surname, Name', or
                   'Smith, J.; Doe, A.' (semicolon-separated) — which
                   the CLI splits for you)
    • editors     (same shape as authors)
    • identifiers (object, e.g. {doi:['10.5555/X']})
    • tags        (array of strings)
    • abstract, source, pages, volume, issue, publisher, ...`,
    )
    ...
    .example(
      'mendeley documents create --data \'{"title":"X","type":"journal","authors":"Smith, J.; Doe, A."}\'',
    )
    ...
    .action(async (_args, flags, out) => {
      ...
      } else if (!flags.title && !flags.type) {
        out.fail(
          '--data is empty; please supply JSON (e.g. --data \'{"title":"\u2026","type":"journal"}\') or use --title/--type',
        );
        return;
      }
  ...
  list of annotation ids.`,
    ...
    .action(async ([id], flags, out) => {
      ...
      const items = flags.all
        ? await collect(
            session.annotations.iter({ document_id: id, pageSize: parseLimit(flags.limit) }),
          )
        : await (async () => {
  ...
  to retrieve the actual contents.`,
    ...
    .action(async ([id], flags, out) => {
      ...
      if (flags.all) {
        const items = await collect(files.iter());
        out.writeList(items);
      } else {
