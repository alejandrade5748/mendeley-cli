...
export function register(program) {
  ...
  produce either JSON or human-readable output.`,
    ...
    .example('mendeley library add-by-catalog-id cat-abcdef12');
  ...
  cmd
    .command('add-by-catalog-id <catalogId>')
    .description('add a catalog document (from `catalog search`) to the library by id')
    .longDescription(
      `Takes a catalog document id (the value of the \`id\` field in
  results from \`mendeley catalog search\`) and adds that document
  to the user library in a single call.  Useful as a fallback when
  the identifier-based paths (add-by-doi / add-by-arxiv) reject a
  real catalog hit (#101) and as a faster path for the common
  "I just searched the catalog, now add this one" workflow.`,
    )
    .option('--folder <id>', 'put the new document into a folder')
    .example('mendeley library add-by-catalog-id cat-abcdef12')
    .example('mendeley library add-by-catalog-id cat-abcdef12 --folder fold-xyz')
    .action(async ([catalogId], flags, out) => {
      const session = await buildSession();
      const catalog = await session.catalog.get(catalogId, { view: 'all' });
      // Strip the catalog `id` field and create the user document with
      // the same metadata.
      const { id, ...metadata } = catalog.toJSON();
      const created = await session.documents.create(metadata);
      if (flags.folder) {
