...
export class Command {
  ...
  async _runInner(args, flags, out) {
    ...
    // `first` is set but is not a known subcommand (#106): reject
    // it with a clean error + "did you mean?" suggestion instead of
    // silently dumping the parent help and exiting 0.
    if (first) {
      const suggestion = suggestClosest(first, this.subcommands.keys());
      const helpCmd = this.isRoot ? 'mendeley --help' : `mendeley ${this._path()} --help`;
      out.fail(
        `unknown subcommand: ${JSON.stringify(first)}` +
          (suggestion ? `\ndid you mean: ${suggestion}?` : '') +
          `\nrun \`${helpCmd}\` for the full list`,
      );
      return;
    }
...
/**
 * Find the closest known candidate name using edit distance, with
 * a shared-prefix fallback. Returns null when nothing is close
 * enough to suggest. Used both for unknown flags (#11) and unknown
 * subcommands (#106).
 *
 * The prefix fallback matters for cases like `library by-identifier`
 * (typed by a user coming from `catalog by-identifier`) — the edit
 * distance to `by-tag` / `by-year` is large, but the shared `by-`
 * prefix is a useful hint.
 */
function suggestClosest(input, candidates) {
  ...
  for (const k of candidates) {
    ...
    // Track the candidate with the longest shared prefix (min 3
    // chars so single-letter coincidences don't fire).
    const prefixLen = commonPrefixLen(input, k);
    if (prefixLen >= 3 && prefixLen > bestPrefixLen) {
      bestPrefixLen = prefixLen;
      bestPrefix = k;
    }
  }
  // Only suggest via edit distance if within a reasonable threshold.
  const threshold = Math.max(2, Math.floor(input.length / 3));
  if (bestDist <= threshold) return best;
  // Otherwise fall back to the prefix match, if any.
  return bestPrefix;
}

/** Length of the common leading substring of two strings. */
...
/**
 * Find the closest known flag name. Thin wrapper over
 * {@link suggestClosest} for flag-specific call sites.
 */
function suggestFlag(input, known) {
  return suggestClosest(input, known);
}
