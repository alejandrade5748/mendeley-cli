...
/**
 * Sentinel thrown by `Output.fail()` to unwind the stack cleanly (#90).
 *
 * Previously `fail()` called `process.exit()`, which trips a libuv
 * `UV_HANDLE_CLOSING` assertion on Windows when fetch keep-alive handles
 * are still open (e.g. after a multi-request command like `catalog lookup`
 * errors on the 2nd request). Throwing this sentinel lets the event loop
 * drain naturally — the top-level handler sets `process.exitCode` and the
 * process exits once pending I/O completes.
 */
...
export class Output {
  ...
  /**
   * Write an error message in the configured format WITHOUT exiting.
   * Used internally by fail() and by the top-level handler.
   */
  writeError(message) {
    if (this.format === 'json') {
  ...
  fail(message, code = 1) {
    ...
    // Throw a sentinel instead of process.exit() so the event loop can
    // drain gracefully (#90). The top-level handler catches this and
